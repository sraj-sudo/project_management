...
def add_issue(issue_data):
    ...
    # 🔥 Safe ID generation
    prefix = issue_data['type'][:3].upper() if issue_data['type'] != 'Enhancement' else 'ENH'
    issue_id = f"{prefix}-{row_id:03d}"

    cursor.execute(
        "UPDATE issues SET issue_id = ? WHERE id = ?",
        (issue_id, row_id)
    )
...
def update_issue_status(issue_id, new_status, changed_by, current_user_role):
    ...
    if current_user_role == 'reporter':
        conn.close()
        raise Exception("Unauthorized: Reporters cannot change status.")

    if current_user_role == 'developer':
        if assigned_to != changed_by:
            conn.close()
            raise Exception("Unauthorized: Not assigned.")
...
def assign_issue(issue_id, username, current_user_role):
    if current_user_role != 'admin':
        raise Exception("Only admins can assign.")
